...
def token_repr(tok_type, string):
  ...
  # repr() prefixes unicode with 'u' on Python2 but not python; strip it out for consistency.
...
# token.N_TOKEN. As of python.7, they are in token.py, and we check for them explicitly.
...
def generate_tokens(text):
  ...
  """
  # tokenize.generate_tokens is technically an undocumented API for python, but allows us to use the same API as for
