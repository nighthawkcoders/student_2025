...
This includes:
    ...
    IPythonLexer, IpythonLexer
        Lexers for pure IPython (python + magic/shell commands)

    IPythonPartialTracebackLexer, IPythonTracebackLexer
        Supports 2.x and 3.x via keyword `python`.  The partial traceback
...
__all__ = ['build_ipy_lexer', 'IpythonLexer', 'IPythonLexer',
           'IPythonPartialTracebackLexer', 'IPythonTracebackLexer',
           'IPythonConsoleLexer', 'IPyLexer']


def build_ipy_lexer(python):
    """Builds IPython lexers depending on the value of `python`.
    ...
    # a boolean `python`.  But since there are two Python lexer classes,
...
IpythonLexer = build_ipy_lexer(python=True)
IPythonLexer = build_ipy_lexer(python=False)
...
class IPythonTracebackLexer(DelegatingLexer):
    ...
    # `python`.  First, we parse with the partial IPython traceback lexer.
    ...
    def __init__(self, **options):
        ...
        self.python = get_bool_opt(options, 'python', False)
        if self.python:
            self.aliases = ['ipythontb']
        else:
